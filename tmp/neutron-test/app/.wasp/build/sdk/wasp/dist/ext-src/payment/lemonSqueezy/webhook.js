...
export const lemonSqueezyWebhook = async (request, response, context) => {
    try {
        ...
        switch (event.meta.event_name) {
            case 'order_created':
                await handleOrderCreated(event, userId, prismaUserDelegate);
                break;
            case 'subscription_created':
                await handleSubscriptionCreated(event, userId, prismaUserDelegate);
                break;
            case 'subscription_updated':
                await handleSubscriptionUpdated(event, userId, prismaUserDelegate);
                break;
            case 'subscription_cancelled':
                await handleSubscriptionCancelled(event, userId, prismaUserDelegate);
                break;
            case 'subscription_expired':
                await handleSubscriptionExpired(event, userId, prismaUserDelegate);
                break;
            default:
...
async function handleOrderCreated(data, userId, prismaUserDelegate) {
    ...
    await updateUserLemonSqueezyPaymentDetails({ lemonSqueezyId, userId, lemonSqueezyCustomerPortalUrl, numOfCreditsPurchased, datePaid }, prismaUserDelegate);
...
async function handleSubscriptionUpdated(data, userId, prismaUserDelegate) {
    ...
    if (status === 'past_due' || status === 'active') {
        await updateUserLemonSqueezyPaymentDetails(Object.assign({ lemonSqueezyId,
            userId, subscriptionPlan: planId, subscriptionStatus: status }, (status === 'active' && { datePaid: new Date() })), prismaUserDelegate);
