...
export async function handleCheckoutSessionCompleted(session, prismaUserDelegate) {
    ...
    return updateUserStripePaymentDetails({ userStripeId, subscriptionPlan, numOfCreditsPurchased, datePaid: new Date() }, prismaUserDelegate);
}
...
function validateUserStripeIdOrThrow(userStripeId) {
    if (!userStripeId)
        throw new HttpError(400, 'No customer id');
